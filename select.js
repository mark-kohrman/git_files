...
var words = ["winner", "winner", "chicken", "dinner", "wayfair"];
var wWords = [];
...
var priceLessThanFive = [];
items.forEach(function (item) {
  if (item.price > 5) {
    priceLessThanFive.push(item);
  }
});
console.log(priceLessThanFive);
...
var objects = [{ name: "chair", price: 100 }, { name: "pencil", price: 1 }, { name: "book", price: 4 }];
var lengthOfObjects = [];
objects.forEach(function (object) {
  if (object["name"].length < 6) {
    lengthOfObjects.push(object);
  }
});
console.log(lengthOfObjects)
...
var numeros = [2, 4, 5, 1, 8, 9, 7];
var oddNumbers = [];
var i = 0;

while (i < numeros.length) {
  if (numeros[i] % 2) {
    oddNumbers.push(numeros[i]);
  }
  i++;
}
